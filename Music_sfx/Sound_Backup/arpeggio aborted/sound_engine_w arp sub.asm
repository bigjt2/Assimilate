...
note_ref .rs 1              ;permanent storeage for current note being played
arp_index .rs 1             ;marks where we are within current arpeggio
arp_loop  .rs 1             ;number of times to loop

stream_waitflag  .rs 6      ;waitflag for each stream: 0=proceed 1=wait



;----------------------------------
;   in arpeggio_index, $80 is infinite loop from beg, $81 is finite loop from beginning
;     and $82 is the byte to repeat final value, $83 kills the process
se_arpeggio:                  ;81b3
  ...
  LDA [curr_arp], y
  CMP #$80
  BEQ .reset    ;start over at beginning, infinitely
  CMP #$81
  BEQ .finite_loop   ;start at beginning, loop finite times  
  CMP #$82
  BEQ .repeat_final  ;stick at last byte in stream
  CMP #$83
  BEQ .rts           ;no arpeggio, skip subroutine
  JMP .adjust        ;if none of these four numbers, must not be an opcode
...
.loop_finish:
  INC arp_index  ;inc twice, skip number of loops to next arp byte
  INC arp_index
  LDY arp_index
  LDA #$FF
  STA arp_loop
...
.repeat_final:  ;repeat the final arpeggio value
  DEC arp_index
  LDY arp_index
  JMP .adjust
...
.adjust:
  LDA note_ref
  CLC
  ADC [curr_arp], y    ;adjust with the arpaggio
...
sound_init:
    ...
    sta sound_sq2_old
    STA arp_loop        ;must init to $FF so arpeggio sub loops correctly
se_silence:
    lda #$30
...
;       X: stream number    
se_fetch_byte:
    LDY arp_index       ;first test if need to reset the arpeggio, otherwise we'll have CHAOS!!!
    LDA [curr_arp], y
    CMP #$83            ;opcode $83 means no arpeggio, so continue
    BEQ .set_pointer
    LDA #$00
    STA arp_index       ;otherwise arpeggio is going and this is new note, so reset arp_index
    LDA #$FF
    STA arp_loop
...
.not_noise:
    ...
    STA note_ref            ;store in this variable for use in arpeggio later
...
se_set_temp_ports:
    ...
    JSR se_arpeggio      ;se_set_temp_ports runs each frame if stream is enabled, so this 
                         ;would be the best place for a new note offset subroutine.
...
song_headers:
    ...
    .word song1_header  ;test song, nuetered
    .word song2_header  ;awesome song
